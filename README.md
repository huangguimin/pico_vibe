...
**多 Agent · 每颗灯一个会话（1.1.26）**：在 AI 氛围 → 显示方式中开启。默认仍是整条灯统一状态；打开后最多 12 颗灯，按客户端 / 会话（以及可靠的子代理 ID）各占一颗灯，思考、执行、空闲、完成、错误、失联和授权各自独立配色与灯效。可改名、定位灯位、反转方向；同时只有一个会话可走触摸授权，其余排队或交回客户端界面。
